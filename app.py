...
EXTRACT_PROMPT = """Extract all flight booking details from the uploaded PDF(s) and return ONLY a raw JSON object.
...
Rules:
- If there is a layover/transfer between flights, set transit on the FIRST flight to:
  {"airport": "Airport name", "duration": "Xhr Ymins", "baggage_status": "checked_through or reclaim"}
- For round trips: use two pages with page_label "Outbound Journey" and "Return Journey"
- For one-way: use one page with page_label as empty string
- brand_hex by airline: Thai Airways=#7B0D1E, SriLankan Airlines=#A6192E, Qatar Airways=#5C0632,
...
- Keep airport names short (max ~30 chars), e.g. "Bandaranaike Intl Arpt" not the full long name
- All times in 24hr HH:MM format"""


def extract_with_gemini(pdf_bytes_list):
...
def generate_eticket_pdf(data):
    ...
    cv.setAuthor(data.get('airline_name', ''))
    ...
    for pi, page in enumerate(pages):
        ...
        for flight in flights:
            ...
            acol = infox - 2*mm - acw - 10*mm
            cv.drawString(acol, bt-2*mm, flight.get('arr_city',''))
            ...
            # dashed arrow
            dep_end   = lx + dcw26 + 3*mm
            arr_start = arr_col - 3*mm
            arrow_y   = bt - 9*mm
            cv.saveState()
            ...
            # right info column
            rows = [
                ("Fare type", flight.get('fare_type','-')),
            ...
            if flight.get('transit'):
                ...
                cv.setFillColor(BRAND)
                cv.rect(MARGIN, sy, 2, sh, fill=1, stroke=0)
        ...
        brows  = [
            (
                f"{f.get('dep_city','')} -> {f.get('arr_city','')} ({f.get('flight_no','')})",
...
def generate():
    ...
    try:
        ...
        data = extract_with_gemini(pdf_bytes_list)
        data.update(overrides)

        # Auto brand colour from airline name if still default
        ...
        return jsonify({'error': f'Could not parse flight data from PDF: {str(e)}'}), 422
